(function ($) {
    ...
    $(document).on('change', '#image_url', function () {
        var output = document.getElementById('about-image-view');
    ...
    $(document).on('click', '.dTableEdit', function () {
        ...
        $('#about-image-view').attr("src", JsManager.BaseUrl() + '/' + rowData.image_url);
        $('#order').val(rowData.order);
        if (rowData['status'] == 1) {
    ...
    var Manager = {
        ...
        Save: function (form) {
            if (Message.Prompt()) {
                ...
                var jsonParam = form;
                var serviceUrl = "website-save-about-us";
        ...
        Update: function (form, id) {
            if (Message.Prompt()) {
                JsManager.StartProcessBar();
                var jsonParam = form;
                var serviceUrl = "website-update-about-us";
                JsManager.SendJsonWithFile("POST", serviceUrl, jsonParam, onSuccess, onFailed);
        ...
        Delete: function (id) {
            if (Message.Prompt()) {
                JsManager.StartProcessBar();
                var jsonParam = { id: id };
                var serviceUrl = "website-delete-about-us";
        ...
        GetDataList: function (refresh) {
            var jsonParam = '';
            var serviceUrl = "website-get-about-us";
