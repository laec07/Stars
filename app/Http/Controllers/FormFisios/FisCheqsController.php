...
use App\Models\FormFisios\FisCheqs;
use Illuminate\Support\Facades\Validator;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\DB;
use Exception;

class FisCheqsController extends Controller
{
    ...
    public function formCheqs(Request $request)
    {
        return view('forms_fisios.cheqs');
    }

    /** Listar todos los registros activos */
    public function getAllformCheqs()
    {
        try {
            $data = FisCheqs::join('cmn_patients', 'fis_cheqs.patient_id', '=', 'cmn_patients.id')
                ->join('users', 'fis_cheqs.user_id', '=', 'users.id')
                ->select(
                    'fis_cheqs.*',
                    'cmn_patients.full_name as customer_name',
                    'users.name as name_user'
                )
                ->where('fis_cheqs.status', 1)
                ->get();
    ...
    public function createformCheqs(Request $request)
    {
        try {
    ...
    public function updateformCheqs(Request $request)
    {
        ...
        try {
            $validator = Validator::make($request->all(), [
                'id' => 'required|integer|exists:fis_cheqs,id',
            ]);
            ...
            $cheqs = FisCheqs::find($request->id);
            if (!$cheqs) {
    ...
    public function deleteformCheqs(Request $request)
    {
        try {
            $validator = Validator::make($request->all(), [
                'id' => 'required|integer|exists:fis_cheqs,id',
    ...
    {
        return collect($request->only((new FisCheqs())->getFillable()))
            ->map(function ($value) {
                if (is_string($value)) {
                    $value = trim($value);
                }
